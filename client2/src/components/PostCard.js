...
const PostCard = ({ post }) => {
   ...
   //comments.map((each)=>console.log(each))
    ...
    return (
        <div className="flex flex-col p-2 bg-white rounded-lg gap-2 m-3 border-2">
            ...
            <div className="flex items-start gap-4">
                ...
                <FaRegCommentDots className='w-8 h-8' onClick={() => setcommtbox(true)} />
            ...
            {commentbox && 
                <div className='bg-white h-20vh'>
                    <div className='flex justify-between'>
                        <h3 className='self-center text-lg text-gray-800 font-bold hover:cursor-pointer'>Comments</h3>
                        <RxCross2 onClick={() => setcommtbox(false)} className='h-5 w-5 text-lg text-gray-800 font-bold'/>
                    </div>
                    <div className='overflow-y-scroll custom-scrollbar scrollbar-none'>
                        <input 
                            type="text" 
                            value={commentText} 
                            onChange={(e) => setCommentText(e.target.value)} 
                            className='w-full bg-transparent outline-none border-gray-300 border-2 rounded-md'
                            placeholder="Add a comment..." 
                        />
                        <button className='h-6 mt-1  w-12 bg-blue-400 rounded-md ' onClick={handleComment}>Post</button>
